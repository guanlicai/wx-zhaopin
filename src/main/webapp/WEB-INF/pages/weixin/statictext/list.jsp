...
<script type="text/javascript">
	$(document).ready(function(){
	  ...
	  $("#modifyBtn").click(function(){
	  	...
	  	if(JS.selectOne()){
	  		window.location.href = "${baseUrl}/admin/statictext/edit.html?id="+JS.getSelect();
	  	}
	  	else{
	  		alert("请选择一条数据");
	  	}
  ...
  <table class="imgtable">
	<thead>
      <tr>
    	...
    	<th width="120px;" style="text-align:center;">编码</th>
    	<th width="300px;" style="text-align:center;">名称</th>
    	<th style="text-align:center;">备注</th>
      </tr>
    </thead>
	<tbody>
	  <c:forEach items="${page.resultList}" var="st" varStatus="s">
	  <tr>
    	<td style="text-align:center;"><input type="checkbox" name="sbox" value="${st.id}" /></td>
    	<td style="text-align:center;">${st.configKey}</td>
    	<td>${st.configVal}</td>
    	<td>${st.remark}</td>
      </tr>
	  </c:forEach>
