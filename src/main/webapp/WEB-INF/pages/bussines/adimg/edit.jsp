...
<script type="text/javascript">
	function submitForm() {
		...
		$.ajax({
			type:'POST',
			url: '${baseUrl}/admin/business/adimg/save.html',
			data: data,
			dataType:"json",
			cache: false,
			success: function(data) {
				if(data.status == "success") {
					alert("保存成功");
					window.location.href = baseUrl + '/admin/business/adimg/list.html';
				}
				else{
					alert(data.message);
				}
	...
	function uploadIcon(){
		var interval = window.setInterval(function(){
			...
			if(filepath != ""){
				...
				$('#myForm').attr("action","${baseUrl }/file/upload.html?path=adimg");
				$('#myForm').ajaxSubmit({
...
<body>
  ...
  <div class="formbody">
	...
	<ul class="forminfo">
      ...
      <li>
      	<label>url</label>
      	<input name="url" value="${entity.url}" type="text" class="dfinput" />
      </li>
