	function treeview(id,path,check,drag){
		var me			= this;
		var target		= null;
		this.nodes		= new node("root","root","roottag").nodes;
		this.id			= id?id:"";
		this.icon		= true;
		this.selected	= null;
		this.path		= path?path:baseURL+"/common/component/tree/images";
		this.selected	= null;
		this.dragTo		= null;
		this.dragFrom	= null;
		...
		for(i in icons){
			...
			this.Ficons[i].src= this.path + "/" + icons[i];
		}
	...
	treeview.prototype.onnodedblclick	= function(sender){return false;}
	...
	//class node
	function node(id,caption,title,url,target,tag,check){
		...
		this.toString = function(tv){
			...
			imgSrc = me.icon?this.icon:(me.nodes.length>0?(me.opened?control.Ficons.open.src:control.Ficons.close.src):control.Ficons.file.src);
			if(imgSrc.indexOf("open.gif") == -1 && imgSrc.indexOf("file.gif") == -1 && imgSrc.indexOf("close.gif") == -1 ){
			...
			var eA				= document.createElement("a");
				...
				var show = "<label for='"+me.id+"'>" + me.caption + "</label>";
				var checked = "";
				if(me.url != -1){
					checked = "checked";
				}
				eA.innerHTML	= "<input type='checkbox' "+checked+" id='"+me.id+"' value='"+me.id+"' parentid='"+me.target+"' name='menubox' onclick='selMenu(this)'>" + (me.caption?show:"");
				
				eA.target		= me.target?me.target	:"#";
				eA.href			= me.action?me.action	:"javascript:";
		...
		this.clone = function(){
			var c = new node(this.id,this.caption,this.title,this.url,this.target,this.tag,this.check);
			copy(this,c);				
			function copy(src,obj){
				for(var i=0;i<src.nodes.length;i++){
					var s = src.nodes.items[i];
					var z = new node(s.id,s.caption,s.title,s.url,s.target,s.tag,s.check);
					z.loaded = false;							
					obj.add(z);
					if(s.nodes.length){
	...
	var children
	function toggleTree(){
		children = tempNode.nodes.items;
		
		if(children.length.length == 0){
			return false;
		}
	...
	function toggleAll(node){
		...
		var children=node.nodes.items;
		var childrenNode;

		for(i=0;i<children.length;i++){
			children[i].toggle();
			
			childrenNode = children[i].nodes.items;
			
			for(var j=0;j<childrenNode.length;j++){
				childrenNode[j].toggle();
				childrenNode[j].toggle();	//close the node
